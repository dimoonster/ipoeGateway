...
sub new {
    ...
    # «апомним инициализирующую информацию
...
sub show_ext_detail() {
    ...
    my @mounths = ("€нвар€", "феврал€", "марта", "апрел€", "ма€", "июн€", "июл€", "августа", "сент€бр€", "окт€бр€", "но€бр€", "декабр€");
    
    my $template = HTML::Template->new(filename => 'tpls/traf_detail.html', case_sensitive => 0);
    $template->param(PAGETITLE => "—татистика пользовани€");
...
sub show_by_mounth() {
    ...
    my @mounths = ("€нварь", "февраль", "март", "апрель", "май", "июнь", "июль", "август", "сент€брь", "окт€брь", "но€брь", "декабрь");
    my @days = (31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
    
    my $template = HTML::Template->new(filename => 'tpls/traf_mounth.html', case_sensitive => 0);
    $template->param(PAGETITLE => "—татистика пользовани€");
    $template->param(SCRIPT_NAME => $ENV{SCRIPT_NAME});
    ...
    for($i=0; $i<$days[$mounth]; $i++) {
      ...
      $temp[$i]{'PAY'} = sprintf("%.2f ћбайт", $traf/1024/1024);

      my $traf = $self->get_day_free_traf($year, $mounth+1, $i+1);
      $temp[$i]{'FREE'} = sprintf("%.2f ћбайт", $traf/1024/1024);
      
      $temp[$i]{'DET1'} = "(<a href=\"/billing.v2.pl?page=traf&act=det2&year=".$year."&mounth=".$mounth."&day=".$i."\">ѕодробнее</a>)";
      $temp[$i]{'DET2'} = "(<a href=\"/billing.v2.pl?page=traf&act=det3&year=".$year."&mounth=".$mounth."&day=".$i."\">ѕодробнее</a>)";
    };
...
sub show_by_year {
  my ($self, $type) = @_;
    ...
    my $template = HTML::Template->new(filename => 'tpls/traf_choose.html', case_sensitive=> 0);
    $template->param(PAGETITLE => "—татистика пользовани€");
    $template->param(SCRIPT_NAME => $ENV{SCRIPT_NAME});
	...
	$tot_pay_traf += $traf;
	$temp_sub[$j]{'PAY_TRAF'} = ($traf==0?'n/a':sprintf("%.2f ћбайт", $traf/1024/1024));

        $traf = $self->get_sum_free_traf($years[$i], $j+1);
	$tot_free_traf += $traf;
	$temp_sub[$j]{'FREE_TRAF'} = ($traf==0?'n/a':sprintf("%.2f ћбайт", $traf/1024/1024));
	
	$temp_sub[$j]{'DET_LINK'} = "/billing.v2.pl?page=traf&act=det1&year=".$years[$i]."&mounth=".$j;
      };
      
      $temp_data[$i]{'TOTAL_FREE'} = sprintf("%.2f ћбайт", $tot_free_traf/1024/1024);
      $temp_data[$i]{'TOTAL_PAY'} = sprintf("%.2f ћбайт", $tot_pay_traf/1024/1024);
      $temp_data[$i]{'TOTAL_TRAF'} = sprintf("%.2f ћбайт", ($tot_free_traf+$tot_pay_traf)/1024/1024);
